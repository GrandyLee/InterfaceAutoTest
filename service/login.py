...
class SSOLogin():
    ...
    def new_ssologin(self, indata, getToken=True):
        encrypted_password = self._sso_pwd_encrypted(indata['reqData']['password'])
        indata['reqData']['password'] = encrypted_password
        # 登录
        res = requests.post(SSOConfig.sso_url, json=indata["reqData"])
        if getToken:
...
class Transport_ssoLogin(SSOLogin):
    def new_sso_login(self, indata, getToken=True):
        ...
        if getToken:
            token = res.json()['data']['token']  # 数据权限会藏在token中
            return token
        else:
            return res.json()
